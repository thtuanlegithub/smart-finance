...
const AddTransactionForm = ({navigation}) => {
  ...
  const hasReminder = useSelector(
    state => state.addTransactionForm.hasReminder,
  );
  const reminderTime = useSelector(
    state => state.addTransactionForm.reminderTime,
  );
  const reminderDate = useSelector(
    state => state.addTransactionForm.reminderDate,
  );
  ...
  const handleSaveTransaction = async () => {
    ...
    const calculateCurrent = (limit, transactions) => {
      ...
      const current = relevantTransactions.reduce(
    ...
    try {
      ...
      let updatedList = data.limitList.map(limit =>
        calculateCurrent(limit, data.transaction),
      );
      updatedList = updatedList.sort(
        (a, b) => b.current / b.amount - a.current / a.amount,
      );

      const grouped = Object.values(
        groupBy(updatedList, ['from_date', 'to_date', 'category_id']),
      );
      ...
      const overLimitGroup = grouped.find(
        group =>
          group.category_id === newTransaction.category_id &&
          group.current >= group.amount / 2,
      );
      if (overLimitGroup) {
        const newNotification = {
          title: 'Cảnh báo chi tiêu',
          message: `Bạn sắp vượt hạn mức chi tiêu ${t(
            overLimitGroup.category_id,
          )} trong khoảng thời gian ${
            overLimitGroup.to_date
          }. Số tiền đã chi: ${formatCurrency(
            overLimitGroup.current,
          )}. Hạn mức: ${formatCurrency(overLimitGroup.amount)}.`,
    ...
    <KeyboardAvoidingView style={styles.container}>
      <ScrollView style={{marginBottom: 60}}>
        ...
        <View style={styles.form}>
          ...
          {type == 'income' && (
            <TouchableOpacity
              onPress={() => navigation.navigate('Calculate Tax')}>
              {hasTax ? (
                <MediumTextIconInput
                  field="tax"
                  value={formatCurrency(
                    calculatePersonalIncomeTax(amount, dependents, insurance),
                  )}
                />
              ) : (
                <MediumTextIconInput field="tax" placeholder={t('no-tax')} />
              )}
            </TouchableOpacity>
          )}
            ...
            <View
              ...
              <Text
                style={[
                  ...
                  {color: colors.green09, padding: 16},
                ]}>
                {t('select-a-day')}
              </Text>
              ...
              <View style={{width: '100%'}}>
                <TouchableOpacity
                  onPress={() => handleSelectTransactionDay(CUSTOM)}
                  style={styles.bottomMenuItemContainer}>
              ...
              <TouchableOpacity
                ...
                <Text
                  style={[
                    typography.RegularInterH3,
                    {color: colors.red01, padding: 16, marginTop: 16},
                  ]}>
                  {t('cancel')}
                </Text>
              </TouchableOpacity>
        ...
        {categoryId === 'debtcollection' && <LoanInformation />}
        {categoryId === 'repayment' && <DebtInformation />}
        <View style={styles.form}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('Reminder')}>
            {hasReminder ? (
              <NoOutlinedMediumTextIconInput
                field="reminder"
                value={reminderTime + ', ' + reminderDate}
              />
            ) : (
              <NoOutlinedMediumTextIconInput
                field="reminder"
                placeholder={t('no-reminder')}
              />
            )}
          </TouchableOpacity>
        </View>
      ...
      <View
        ...
        <W1Button title={t('save')} onPress={handleSaveTransaction} />
      </View>
