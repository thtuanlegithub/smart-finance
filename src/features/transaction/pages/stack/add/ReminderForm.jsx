...
const ReminderForm = () => {
  ...
  const currentTransactionCRUDAction = useSelector(
    state => state.transaction.currentTransactionCRUDAction,
  );
  if (currentTransactionCRUDAction === 'create') {
    var hasReminder = useSelector(
      state => state.addTransactionForm.hasReminder,
    );
    var reminderTime = useSelector(
      state => state.addTransactionForm.reminderTime,
    );
    var reminderDate = useSelector(
      state => state.addTransactionForm.reminderDate,
    );
  } else if (currentTransactionCRUDAction === 'update') {
    var hasReminder = useSelector(
      state => state.updateTransactionForm.hasReminder,
    );
    var reminderTime = useSelector(
      state => state.updateTransactionForm.reminderTime,
    );
    var reminderDate = useSelector(
      state => state.updateTransactionForm.reminderDate,
    );
  }
        ...
        <View
          ...
          <View style={styles.dateTimePickerBtn}>
            {reminderTime ? (
              ...
              <TouchableOpacity onPress={() => setTimePickerOpen(true)}>
                <Text style={styles.dateTimePickerBtnText}>
                  {t('select-time')}
                </Text>
              </TouchableOpacity>
            )}
